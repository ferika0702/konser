        ...
        <!-- <table class="table table-striped table-sm"> -->
          ...
          <tbody>
            <tr>
              ...
              <td>{{ $item -> kode}}</td>
              <td>
              <form action="{{ url('/penonton/selesai/' . $item->id) }}" method="GET" onsubmit="return confirm('Cek In Sekarang?')">
                    <button type="submit" class="btn btn-danger">Waiting</button>
              </td>
</form>
              <td>
                 <form action="{{ route('penonton.destroy', $item->id)  }}" method="post" onsubmit="return confirm('Yakin Hapus Data?')">
